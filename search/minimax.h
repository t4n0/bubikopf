...
Evaluation minimax(Node& node, const uint8_t depth, const Evaluation alpha_parent, const Evaluation beta_parent)
{
    ...
    else if (node.position_.GetTurn() == Player::max)
    {
        ...
        {
            Evaluation eval = minimax<Behaviour>(*child, depth - 1, alpha, beta_parent);
    ...
    else
    {
        ...
        for (const std::unique_ptr<Node>& child : node.children_)
        {
            Evaluation eval = minimax<Behaviour>(*child, depth - 1, alpha_parent, beta);
