...
template <typename Method = ConcreteEvaluation>
Evaluation minimax(const Node& node, const uint8_t depth, const Player turn,
  ...
  if ((depth == 0) || !node.children_.size()) {
    ...
    for (const std::unique_ptr<Node>& child : node.children_) {
      Evaluation eval =
          minimax<Method>(*child, depth - 1, Player::max, alpha, beta);
