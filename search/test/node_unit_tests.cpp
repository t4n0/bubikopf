...
{
    const Node node{};
    std::cout << node.position_.board_;
    // check manually
}
...
TEST(CountNodesTest, Given7Children_Expect8)
{
    ...
    std::generate(
        root_node.children_.begin(), root_node.children_.end(), []() { return std::make_unique<Node>(Position{}); });
...
TEST(CountNodesTest, Given2ChildrenAnd4GrandChildren_Expect7)
{
    // Setup
    Node root_node{Position{}};
    root_node.children_.push_back(std::make_unique<Node>(Position{}));
    root_node.children_.push_back(std::make_unique<Node>(Position{}));
    root_node.children_.at(0)->children_.push_back(std::make_unique<Node>(Position{}));
    root_node.children_.at(0)->children_.push_back(std::make_unique<Node>(Position{}));
    root_node.children_.at(1)->children_.push_back(std::make_unique<Node>(Position{}));
    root_node.children_.at(1)->children_.push_back(std::make_unique<Node>(Position{}));
