...
std::unique_ptr<Node> MakeChildWithUniqueIdHiddenInPliesVariable(const Node& node)
{
    std::unique_ptr<Node> child = std::make_unique<Node>(Position{});
    child->position_.static_plies_ = ++GLOBAL_NODE_IDENTIFIER;
    child->position_.plies_ = node.position_.plies_ + 1;
    return child;
}
...
struct MinimaxTest_Fixture : public ::testing::TestWithParam<MinimaxTestParameters>
{
    void SetUp() override
    {
...
TEST_P(MinimaxTest_Fixture, GivenDepths_ExpectCutOfAtSmallerDepth)
{
    ...
    // Call
    minimax<AllNodesEvaluateToSameValue>(root_node,
                                         GetParam().search_depth,
                                         AllNodesEvaluateToSameValue::arbitrary_value,
                                         AllNodesEvaluateToSameValue::arbitrary_value);

    // Expect
    const std::size_t number_of_evaluated_nodes{ORDER_OF_NODE_EVALUATION.size()};
    EXPECT_EQ(number_of_evaluated_nodes, GetParam().expected_number_of_evaluated_nodes);
}
...
struct LeafNodesEvaluateToDecreasingValues
{
    static float GetNodeValue(int identifier)
    {
        const std::map<int, float> map_id_value{
            {5, -1.0F}, {6, -2.0F}, {7, -3.0F}, {8, -4.0F}, {11, -5.0F}, {12, -6.0F}, {13, -7.0F}, {14, -8.0F}};
...
struct LeafNodesEvaluateToTypicalValues
{
    ...
    {
        const std::map<int, float> map_id_value{
            {5, -1.0F}, {6, 3.0F}, {7, 5.0F}, {8, 42.0F}, {11, -4.0F}, {12, -6.0F}, {13, 43.0F}, {14, 44.0F}};
        return map_id_value.at(identifier);
    }
    static std::vector<int> GetExpectedEvaluationOrder()
    {
...
using EvaluateMocks = ::testing::
    Types<LeafNodesEvaluateToDecreasingValues, LeafNodesEvaluateToIncreasingValues, LeafNodesEvaluateToTypicalValues>;
TYPED_TEST_SUITE(MinimaxTest_TypedFixture, EvaluateMocks);

TYPED_TEST(MinimaxTest_TypedFixture, GivenInjectedNodeEvaluation_ExpectEvaluationOrderAndFinalEvaluation)
{
    // Setup
    using EvaluateMock = TypeParam;

    // Call
    const Evaluation returned_evaluation =
        minimax<EvaluateMock>(TestFixture::node_, TestFixture::depth_, MIN_EVAL, MAX_EVAL);

    // Expect
    EXPECT_FLOAT_EQ(std::get<float>(returned_evaluation), TypeParam::GetExpectedFinalEvaluation());
