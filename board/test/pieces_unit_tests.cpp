...
TEST_F(BlackPawnFindPlies_Fixture, GivenNoFreeSquareInfront_ExpectNoPlies)
{
    ...
    const std::vector<Position> returned_positions{
        position_.board_.Get(black_pawn_location)->FindPlies(ToIdx(black_pawn_location), position_)};
...
TEST_F(BlackPawnFindPlies_Fixture, GivenWhitePawn_ExpectNoPlies)
{
    ...
    // Call
    const std::vector<Position> returned_positions{
        position_.board_.Get(white_pawn_location)->FindPlies(ToIdx(white_pawn_location), position_)};

    // Expect
    EXPECT_TRUE(returned_positions.empty());
}

TEST_F(BlackPawnFindPlies_Fixture, GivenLocationNotOnStartRankAndFreeSquareInfront_ExpectSingleStepOnly)
{
    // Setup
    ...
    EXPECT_TRUE(returned_position.board_.Get(location_behind_black_pawn)->IsEmpty());
    EXPECT_TRUE(returned_position.board_.Get(black_pawn_location)->IsEmpty());
    EXPECT_EQ(returned_position.board_.Get(free_square)->GetId(), SquareId::BlackPawn);
    EXPECT_FALSE(returned_position.en_passant_);
    EXPECT_EQ(returned_position.previous_move_, "d4d3");
}

TEST_F(BlackPawnFindPlies_Fixture, GivenFreeSquareOnPromotionRank_ExpectAllPromotions)
{
    // Setup
    ...
    EXPECT_EQ(returned_positions.at(0).board_.Get(promotion_square)->GetId(), SquareId::BlackQueen);
    EXPECT_EQ(returned_positions.at(0).previous_move_, "d2d1q");
    EXPECT_EQ(returned_positions.at(1).board_.Get(promotion_square)->GetId(), SquareId::BlackRook);
    EXPECT_EQ(returned_positions.at(1).previous_move_, "d2d1r");
    EXPECT_EQ(returned_positions.at(2).board_.Get(promotion_square)->GetId(), SquareId::BlackKnight);
    EXPECT_EQ(returned_positions.at(2).previous_move_, "d2d1n");
    EXPECT_EQ(returned_positions.at(3).board_.Get(promotion_square)->GetId(), SquareId::BlackBishop);
    EXPECT_EQ(returned_positions.at(3).previous_move_, "d2d1b");
}

TEST_F(BlackPawnFindPlies_Fixture, GivenLocationOnStartRankAndTwoFreeSquaresInfront_ExpectAlsoDoubleStep)
{
    // Setup
    ...
    EXPECT_TRUE(returned_positions.at(0).board_.Get(double_step_target)->IsEmpty());
    EXPECT_EQ(returned_positions.at(0).board_.Get(single_step_target)->GetId(), SquareId::BlackPawn);
    EXPECT_EQ(returned_positions.at(0).previous_move_, "e7e6");
    EXPECT_TRUE(returned_positions.at(1).board_.Get(single_step_target)->IsEmpty());
    EXPECT_EQ(returned_positions.at(1).board_.Get(double_step_target)->GetId(), SquareId::BlackPawn);
...
TEST_F(BlackPawnFindPlies_Fixture, GivenOneCapturePossible_ExpectOnePly)
{
    ...
    position_.board_.Set(black_pawn_location, Pawn::OfSide(Player::min));
    position_.board_.Set(black_piece_on_capture_location, Knight::OfSide(Player::min));
    position_.board_.Set(white_blocking_piece_location, King::OfSide(Player::max));
    ...
    EXPECT_EQ(returned_positions.size(), 1);
    EXPECT_TRUE(returned_position.board_.Get(black_pawn_location)->IsEmpty());
    EXPECT_EQ(returned_position.board_.Get(white_blocking_piece_location)->GetId(), SquareId::WhiteKing);
    EXPECT_EQ(returned_position.board_.Get(white_hanging_piece_location)->GetId(), SquareId::BlackPawn);
    EXPECT_EQ(returned_position.board_.Get(black_piece_on_capture_location)->GetId(), SquareId::BlackKnight);
...
TEST_F(BlackPawnFindPlies_Fixture, GivenEnPassant_ExpectCapture)
{
    ...
    // Call
    const std::vector<Position> returned_positions{
        position_.board_.Get(black_pawn)->FindPlies(ToIdx(black_pawn), position_)};
    ...
    EXPECT_EQ(returned_position.board_.Get(en_passant)->GetId(), SquareId::BlackPawn);
    EXPECT_EQ(returned_position.board_.Get(white_blocking_piece)->GetId(), SquareId::WhiteKnight);
...
TEST_F(WhitePawnFindPlies_Fixture, GivenLocationNotOnStartRankAndFreeSquareInfront_ExpectSingleStepOnly)
{
    // Setup
    ...
    EXPECT_EQ(returned_positions.size(), 1);
    const Position& returned_position = returned_positions.front();
    EXPECT_TRUE(returned_position.board_.Get(location_behind_white_pawn)->IsEmpty());
    EXPECT_TRUE(returned_position.board_.Get(white_pawn_location)->IsEmpty());
    EXPECT_EQ(returned_position.board_.Get(free_square)->GetId(), SquareId::WhitePawn);
    EXPECT_FALSE(returned_position.en_passant_);
    EXPECT_EQ(returned_position.previous_move_, "d4d5");
}

TEST_F(WhitePawnFindPlies_Fixture, GivenFreeSquareOnPromotionRank_ExpectAllPromotions)
{
    // Setup
    ...
    EXPECT_EQ(returned_positions.at(0).board_.Get(promotion_square)->GetId(), SquareId::WhiteQueen);
    EXPECT_EQ(returned_positions.at(0).previous_move_, "d7d8q");
    EXPECT_EQ(returned_positions.at(1).board_.Get(promotion_square)->GetId(), SquareId::WhiteRook);
    EXPECT_EQ(returned_positions.at(1).previous_move_, "d7d8r");
    EXPECT_EQ(returned_positions.at(2).board_.Get(promotion_square)->GetId(), SquareId::WhiteKnight);
    EXPECT_EQ(returned_positions.at(2).previous_move_, "d7d8n");
    EXPECT_EQ(returned_positions.at(3).board_.Get(promotion_square)->GetId(), SquareId::WhiteBishop);
    EXPECT_EQ(returned_positions.at(3).previous_move_, "d7d8b");
}

TEST_F(WhitePawnFindPlies_Fixture, GivenLocationOnStartRankAndTwoFreeSquaresInfront_ExpectAlsoDoubleStep)
{
    // Setup
    ...
    EXPECT_TRUE(returned_positions.at(0).board_.Get(double_step_target)->IsEmpty());
    EXPECT_EQ(returned_positions.at(0).board_.Get(single_step_target)->GetId(), SquareId::WhitePawn);
    EXPECT_EQ(returned_positions.at(0).previous_move_, "e2e3");
    EXPECT_TRUE(returned_positions.at(1).board_.Get(single_step_target)->IsEmpty());
    EXPECT_EQ(returned_positions.at(1).board_.Get(double_step_target)->GetId(), SquareId::WhitePawn);
    EXPECT_EQ(returned_positions.at(1).previous_move_, "e2e4");
}

TEST_F(WhitePawnFindPlies_Fixture, GivenDoubleStep_ExpectEnPassantOnlyForDoubleStep)
{
    // Setup
    const Coordinate white_pawn_location{4, 6};
    const Coordinate single_step_target{4, 5};
...
TEST_F(WhitePawnFindPlies_Fixture, GivenOneCapturePossible_ExpectOnePly)
{
    ...
    position_.board_.Set(white_pawn_location, Pawn::OfSide(Player::max));
    position_.board_.Set(white_piece_on_capture_location, Knight::OfSide(Player::max));
    position_.board_.Set(black_blocking_piece_location, King::OfSide(Player::min));
    ...
    const Position& returned_position = returned_positions.front();
    EXPECT_TRUE(returned_position.board_.Get(white_pawn_location)->IsEmpty());
    EXPECT_EQ(returned_position.board_.Get(black_blocking_piece_location)->GetId(), SquareId::BlackKing);
    EXPECT_EQ(returned_position.board_.Get(black_hanging_piece_location)->GetId(), SquareId::WhitePawn);
    EXPECT_EQ(returned_position.board_.Get(white_piece_on_capture_location)->GetId(), SquareId::WhiteKnight);
...
TEST_F(WhitePawnFindPlies_Fixture, GivenEnPassant_ExpectCapture)
{
    ...
    // Call
    const std::vector<Position> returned_positions{
        position_.board_.Get(white_pawn)->FindPlies(ToIdx(white_pawn), position_)};
    ...
    EXPECT_TRUE(returned_position.board_.Get(white_pawn)->IsEmpty());
    EXPECT_TRUE(returned_position.board_.Get(black_pawn)->IsEmpty());
    EXPECT_EQ(returned_position.board_.Get(en_passant)->GetId(), SquareId::WhitePawn);
    EXPECT_EQ(returned_position.board_.Get(black_blocking_piece)->GetId(), SquareId::BlackKnight);
...
TEST_F(PiecePlies_Fixture, GivenKnightOfPlayerWhoIsNotOnTurn_ExpectNoPlies)
{
    ...
    // Call
    const std::vector<Position> returned_positions{
        position_.board_.Get(knight_location)->FindPlies(ToIdx(knight_location), position_)};
...
TEST_F(PiecePlies_Fixture, GivenKnightInCornerAndPiecesOnTargetSquares_ExpectCapture)
{
    // Setup
    const Coordinate knight_location{0, 0};
...
TEST_F(PiecePlies_Fixture, GivenBishopInCenterWithOneCapturePossible_ExpectAppropriateNumberOfPlies)
{
    // Setup
    ...
    // Call
    const std::vector<Position> returned_positions{
        position_.board_.Get(bishop_location)->FindPlies(ToIdx(bishop_location), position_)};
...
TEST_F(PiecePlies_Fixture, GivenRookInCenterWithOneCapturePossible_ExpectAppropriateNumberOfPlies)
{
    // Setup
    ...
    // Call
    const std::vector<Position> returned_positions{
        position_.board_.Get(rook_location)->FindPlies(ToIdx(rook_location), position_)};
...
TEST_F(PiecePlies_Fixture, GivenQueenInCenterWithOneCapturePossible_ExpectAppropriateNumberOfPlies)
{
    // Setup
    ...
    // Call
    const std::vector<Position> returned_positions{
        position_.board_.Get(queen_location)->FindPlies(ToIdx(queen_location), position_)};
...
TEST_F(PiecePlies_Fixture, GivenKingOnEdgeWithOneCapturePossible_ExpectAppropriateNumberOfPlies)
{
    // Setup
    ...
    // Call
    const std::vector<Position> returned_positions{
        position_.board_.Get(king_location)->FindPlies(ToIdx(king_location), position_)};
