...
int main()
{
    ...
    while (true)
    {
        ...
        {
            PrintBoardWithPlyCaption(board_after_human_move, node->position_.plies_ - 1);
        }
        ...
        for (std::size_t idx{0}; idx < branch_evaluations.size(); idx++)
        {
            branch_evaluations.at(idx) =
                Chess::minimax(*(node->children_.at(idx)), DEPTH, Chess::MIN_EVAL, Chess::MAX_EVAL);
            std::cout << "Option " << static_cast<int>(idx) << " with evaluation of " << branch_evaluations.at(idx)
                      << ":\n";
            std::cout << node->children_.at(idx)->position_.board_;
        }
        const auto min_element_itr = std::min_element(branch_evaluations.begin(), branch_evaluations.end());
        if (min_element_itr != branch_evaluations.end())
        {
            std::size_t ai_move_idx = static_cast<std::size_t>(min_element_itr - branch_evaluations.begin());
            std::cout << "Ai's choice with evaluation of " << branch_evaluations.at(ai_move_idx) << ":\n";
